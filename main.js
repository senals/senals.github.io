...
// Loader Animation
const timelineSquare1 = gsap.timeline({ repeat: -1 })
  .to(".square1", {
    top: "50px",
    left: "0px",
    duration: 0.5,
...
const timelineSquare4 = gsap.timeline({ repeat: -1 })
  .to(".square4", {
    bottom: "50px",
    right: "0px",
    duration: 0.5,
  ...
  setTimeout(function () {
    gsap.to(".loader", {
      ...
      onComplete: function () {
        ...
        document.querySelector(".loader").style.display = "none"; // Hide instead of removing
      },
    });
...
document.addEventListener("DOMContentLoaded", function () {
  // Slider Animation
  function sliderAnim() {
    const tween = gsap.to(".slider_text", {
      xPercent: -200,
      repeat: -1,
      duration: 10,
      ease: "linear",
  ...
  // Scroll-to Effect
  function attachScrollAnimation(selector, targetId, offsetY = 120) {
    document.querySelectorAll(selector).forEach((element) => {
      element.addEventListener("click", (event) => {
        event.preventDefault();
        gsap.to(window, {
          duration: 0.6,
          scrollTo: { y: targetId, offsetY: offsetY },
          ease: "power2.inOut",
        });
      });
    });
    ...
    button.addEventListener("click", () => {
      ...
      setTimeout(() => {
        if (document.documentElement.getAttribute("data-theme") === "dark") {
          ...
          logos.forEach((logo) => {
            logo.src = "/images/logo.svg"; // Change logos to light mode
          });
        } else {
          document.documentElement.setAttribute("data-theme", "dark");
          logos.forEach((logo) => {
            logo.src = "/images/logo_white.svg"; // Change logos to dark mode
          });
        }
  ...
  gsap.delayedCall(1.5, function () {
    ...
    elements.forEach(function (element, index) {
      ...
      }, index * 300); // 0.3 seconds delay for each subsequent element
    });
  });
  ...
  burgerMenu.addEventListener("click", () => {
    burgerSlider.classList.toggle("sliding_burger");
    burgerMenu.classList.toggle("burger_rotate");
  });

  // Paragraph Appear Animation
